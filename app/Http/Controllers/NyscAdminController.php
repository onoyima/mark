...
<<<<<<< Updated upstream
    ...
    private function canViewDuplicates($user): bool
    {
        if (!$user) return false;
        return (int)($user->id ?? 0) === 596;
    }
    ...
    public function getPaymentStatistics(\Illuminate\Http\Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $canDup = $this->canViewDuplicates($user);
        ...
        if (!$canDup) { $duplicatesFilter = 'exclude'; }
        ...
        $duplicateStudentsCount = $canDup ? $duplicates->count() : 0;
        $duplicatePaymentsCount = $canDup ? $duplicates->map->count()->sum() : 0;
        $duplicateAmount = $canDup ? $duplicates->flatten()->sum('amount') : 0;
    ...
    public function exportPaymentStatistics(\Illuminate\Http\Request $request)
    {
        ...
        if ($format === 'excel') {
            ...
            $excelHeaders = ['student_id', 'matric_no', 'name', 'department', 'payment_count', 'total_amount', 'late_fee_count', 'normal_fee_count'];
            if ($canDup) { $excelHeaders[] = 'duplicate_count'; }
            ...
            foreach ($grouped as $sid => $g) {
                ...
                if ($canDup) { $sheet->setCellValue("I{$rowIndex}", "=E{$rowIndex}-1"); }
                $rowIndex++;
            }
            // Summary on a dedicated sheet using whole-column formulas for dynamic recalculation
            ...
            $summarySheet->setCellValue('A2', 'Total Students Paid');
            $summarySheet->setCellValue('B2', '=COUNTA(Payments!A:A)-1');
            $summarySheet->setCellValue('A3', 'Total Amount Paid');
            $summarySheet->setCellValue('B3', '=SUM(Payments!F:F)');
            $summaryRow = 4;
            if ($canDup) {
                $summarySheet->setCellValue('A4', 'Total Duplicates');
                $summarySheet->setCellValue('B4', '=SUM(Payments!I:I)');
                $summaryRow = 5;
            }
            $summarySheet->setCellValue('A' . $summaryRow, 'Total Normal Fees');
            $summarySheet->setCellValue('B' . $summaryRow, '=SUM(Payments!H:H)');
            $summaryRow++;
            $summarySheet->setCellValue('A' . $summaryRow, 'Total Late Fees');
            $summarySheet->setCellValue('B' . $summaryRow, '=SUM(Payments!G:G)');
            ...
            $callback = function () use ($payments, $canDup) {
                $f = fopen('php://output', 'w');
                fprintf($f, chr(0xEF) . chr(0xBB) . chr(0xBF));
                $csvHeaders = ['student_id', 'matric_no', 'name', 'department', 'payment_count', 'total_amount', 'late_fee_count', 'normal_fee_count'];
                if ($canDup) { $csvHeaders[] = 'duplicate_count'; }
                ...
                // Append summary rows with whole-column formulas for dynamic recalculation in Excel
                fputcsv($f, []);
                fputcsv($f, ['Summary', 'Metric', 'Value']);
                fputcsv($f, ['', 'Total Students Paid', '=COUNTA(A:A)-1']);
                fputcsv($f, ['', 'Total Amount Paid', '=SUM(F:F)']);
                if ($canDup) { fputcsv($f, ['', 'Total Duplicates', '=SUM(I:I)']); }
                fputcsv($f, ['', 'Total Normal Fees', '=SUM(H:H)']);
                fputcsv($f, ['', 'Total Late Fees', '=SUM(G:G)']);
                fclose($f);
            };
            return response()->stream($callback, 200, $headers);
        }
    ...
    public function hideStudentsPayments(\Illuminate\Http\Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        if ($mode === 'duplicates' && !$this->canViewDuplicates($user)) {
            return response()->json(['success' => false, 'message' => 'Forbidden'], 403);
        }
...
=======
>>>>>>> Stashed changes
     * Get all NYSC sessions
     */
    public function getSessions()
    {
        $sessions = NyscSession::orderBy('created_at', 'desc')->get();
        return response()->json([
            'success' => true,
            'data' => $sessions
        ]);
    ...
    /**
     * Create a new NYSC session
     */
    public function createSession(Request $request)
    {
        $validated = $request->validate([
            'name' => 'required|string|unique:nysc_sessions,name|max:255',
            'code' => 'nullable|string|unique:nysc_sessions,code|max:50',
        ...
        return response()->json([
            'success' => true,
            'message' => 'NYSC Session created successfully.',
            'data' => $session
        ], 201);
    }

    /**
     * Activate a specific NYSC session
     */
    public function activateSession($id)
    {
        $sessionToActivate = NyscSession::find($id);
        ...
        try {
            ...
            NyscSession::where('is_active', true)->update(['is_active' => false]);

            // Activate the selected session
            $sessionToActivate->is_active = true;
            $sessionToActivate->save();
